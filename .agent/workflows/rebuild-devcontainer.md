...
```bash
docker compose -p data-structures-and-algorithms_devcontainer -f .devcontainer/docker-compose.yaml build --no-cache
```

3. Start the container in detached mode
```bash
docker compose -p data-structures-and-algorithms_devcontainer -f .devcontainer/docker-compose.yaml up -d
```
